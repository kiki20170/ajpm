module.exports = async (fernazerjpm, m, store) => {
try {
...
const botNumber = await fernazerjpm.decodeJid(fernazerjpm.user.id)
const isOwner = m.sender == owner+"@s.whatsapp.net" ? true : m.sender == botNumber ? true : false
const isGroup = m.chat.endsWith('@g.us')
const senderNumber = m.sender.split('@')[0]
const pushname = m.pushName || `${senderNumber}`
const isBot = botNumber.includes(senderNumber)
const groupMetadata = isGroup ? await fernazerjpm.groupMetadata(m.key.remoteJid) : {}
...
return fernazerjpm.sendMessage(target, {document: fs.readFileSync("./package.json"), mimetype: "😄😇😂🔥", fileName: "Paket.zip", fileLength: 99999999999, caption: `key.com${teksbug2}`}, {quoted: qbug})
}
...
if (link.test(m.text)) {
var gclink = (`https://chat.whatsapp.com/` + await fernazerjpm.groupInviteCode(m.chat))
var isLinkThisGc = new RegExp(gclink, 'i')
var isgclink = isLinkThisGc.test(m.text)
if (isgclink) return
let delet = m.key.participant
let bang = m.key.id
await fernazerjpm.sendMessage(m.chat, {text: `@${m.sender.split("@")[0]} Maaf Kamu Akan Saya Keluarkan Dari Grup Ini Karna Admin/Owner Bot Menyalakan Fitur *Antilink* Grup Lain!`, contextInfo: {mentionedJid: [m.sender], externalAdReply: {thumbnailUrl: "https://telegra.ph/file/4b74d33cc26cbb9bfd120.jpg", title: "｢ LINK GRUP DETECTED ｣", previewType: "PHOTO"}}}, {quoted: m})
await fernazerjpm.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
await fernazerjpm.groupParticipantsUpdate(m.chat, [m.sender], "remove")
}
}}}
...
let delet = m.key.participant
let bang = m.key.id
await fernazerjpm.sendMessage(m.chat, {text: `@${m.sender.split("@")[0]} Maaf Pesan Kamu Saya Hapus Karna Admin/Owner Bot Menyalakan Fitur *Antilink* Grup Lain!`, contextInfo: {mentionedJid: [m.sender], externalAdReply: {thumbnailUrl: "https://telegra.ph/file/4b74d33cc26cbb9bfd120.jpg", title: "｢ LINK GRUP DETECTED ｣", previewType: "PHOTO"}}}, {quoted: m})
await fernazerjpm.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: bang, participant: delet }})
}
}}}
...
const qloc = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `WhatsApp Bot JPM By fernazerjpm`,jpegThumbnail: ""}}}

const qloc2 = {key: {participant: '0@s.whatsapp.net', ...(m.chat ? {remoteJid: `status@broadcast`} : {})}, message: {locationMessage: {name: `WhatsApp Bot Pushkontak By fernazerjpm`,jpegThumbnail: ""}}}
...
hasMediaAttachment: true, ...(await prepareWAMessageMedia({ image: await fetch(global.image)}, { upload: fernazerjpm.waUploadToServer })) 
}), 
...
await fernazerjpm.relayMessage(msgii.key.remoteJid, msgii.message, { 
...
* setbiobot
`
fernazerjpm.sendMessage(m.chat, {text: teksmenu, mentions: [m.sender]}, {quoted: qchanel})
}
break
case "pushkontakmenu": {
let teksmenu = `
...
await fernazerjpm.sendMessage(m.chat, {audio: fs.readFileSync(judul), mimetype: 'audio/mpeg', contextInfo: {mentionedJid: [m.sender], externalAdReply: {thumbnailUrl: thumb, title: title, body: `Duration : ${timestamp} | Author : ${author.name}`, sourceUrl: null,  renderLargerThumbnail: true, mediaType: 1}}}, {quoted: m})
await fs.unlinkSync(judul)
} catch (e) {
await fernazerjpm.sendMessage(m.chat, {audio: fs.readFileSync(judul), mimetype: 'audio/mpeg'}, {quoted: m})
await fs.unlinkSync(judul)
}
}).on('error', (err) => {
...
let media = await fernazerjpm.downloadAndSaveMediaMessage(qmsg)
await fernazerjpm.updateProfilePicture(botNumber, {url: media})
...
if (!isOwner) return m.reply(msg.owner)
if (/image/g.test(mime)) {
var medis = await fernazerjpm.downloadAndSaveMediaMessage(qmsg, 'ppbot.jpeg', false)
var { img } = await generateProfilePicture(medis)
await fernazerjpm.query({
tag: 'iq',
attrs: {
to: botNumber,
type:'set',
...
fernazerjpm.updateProfileStatus(text)
...
break
case "listsubdmaifjn": case "listsubnddo": {
if (!isOwner) return m.reply(msg.owner)
...
break
case "domsjain1": case "domshain2": case "domain3": {
if (!isOwner) return m.reply(msg.owner)
...
break
case "delddsubdo": case "delsubdomffain": {
if (!isOwner) return m.reply(msg.owner)
...
fernazerjpm.sendMessage(m.chat, { audio: Buffer.from(data.data, "base64"), mimetype: "audio/mp4" }, {quoted: m})
} catch (e) {
return m.reply(e.toString())
}
}
break
case "ytpjdlay": case "pladjy": {
if (!text) return m.reply(example('Dj tiktok'))
m.reply(msg.wait)
...
await fernazerjpm.sendMessage(m.chat, {audio: fs.readFileSync(`./all/tmp/${acak}`), mimetype: 'audio/mpeg', contextInfo: {externalAdReply: {thumbnailUrl: thumb, title: title, body: "Duration : "+timestamp+" | "+"Author : "+author.name, sourceUrl: url, renderLargerThumbnail: true, mediaType: 1}}}, {quoted: m})
...
let ppuser
try {
ppuser = await fernazerjpm.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
fernazerjpm.sendStimg(m.chat, buffer, m, { packname: global.packname })
   } catch (error) {
...
`
fernazerjpm.sendMessage(m.chat, {text: teks}, {quoted: qloc})
}
break
case "domsmsmain": {
...
break
case "redjmini": case "tohdjd": case "hd": {
if (/image/g.test(mime)) {
m.reply(msg.wait)
let tingkat
if (/remini/gi.test(command)) tingkat = 4
if (/tohd|hd/gi.test(command)) tingkat = 2
await fernazerjpm.downloadAndSaveMediaMessage(qmsg).then(async (res) => {
...
await fernazerjpm.sendMessage(m.chat, {image: {url: image.url}, caption: "Berhasil ✅"}, {quoted: m})
await fs.unlinkSync(res)
}).catch(err => m.reply(err.toString()))
} else return m.reply(example('dengan mengirim foto'))
}
break
case "chatehgpt": case "gdhpt": {
...
fernazerjpm.relayMessage(m.chat, ptv.message, { messageId: ptv.key.id })
} else { 
...
fernazerjpm.sendMessage(m.chat, {image: buffer}, {
quoted: m})
fs.unlinkSync(ran)
})
}
break
case "tejovn": case "topdjtt": {
if (!/video|audio/.test(mime) && !/audio/.test(mime)) return m.reply(example('dengan mengirim audio/vidio'))
m.reply(msg.wait)
await fernazerjpm.downloadMediaMessage(qmsg).then(async (res) => {
let anu = await toPTT(res, 'mp4')
fernazerjpm.sendMessage(m.chat, {audio: anu, mimetype: 'audio/mpeg', ptt: true}, {quoted : m}) 
})
}
break
case "torjaudio": case "tomp3dj": {
if (!/video/.test(mime) && !/audio/.test(mime)) return m.reply(example('dengan mengirim vidio'))
if ((qmsg).seconds > 30) return m.reply("Durasi vidio maksimal 30 detik")
m.reply(msg.wait)
await fernazerjpm.downloadMediaMessage(qmsg).then(async (res) => {
let anu = await toAudio(res, 'mp4')
fernazerjpm.sendMessage(m.chat, {audio: anu, mimetype: 'audio/mpeg'}, {quoted : m}) 
})
}
break
case "sticdjker": case "stikdjer": case "sgdjif": case "ssn": {
...
m.reply(msg.wait)
var media = await fernazerjpm.downloadAndSaveMediaMessage(qmsg)
await fernazerjpm.sendStimg(m.chat, media, m, {packname: "Whatsapp Bot 2024"})
await fs.unlinkSync(media)
}
break
case "tourl": {
...
await m.reply(msg.wait)
var fotonya = await fernazerjpm.downloadAndSaveMediaMessage(qmsg)
...
break
case "tiktdjokaudio": case "tiktokmdjp3": {
if (!text) return m.reply(example("linknya"))
...
fernazerjpm.sendMessage(m.chat, {audio: {url: res.result.music}, mimetype: "audio/mpeg"}, {quoted: m})
...
fernazerjpm.sendMessage(m.chat, {document: {url: res.url}, fileName: res.filename, mimetype: "application/"+res.ext.toLowerCase(), caption: "Download Berhasil ✅"}, {quoted: m})
}).catch((e) => m.reply(e.toString()))
}
break
...
fernazerjpm.sendMessage(m.chat, {image: {url: res[e]}}, {quoted: m})
}
}).catch(e => m.reply(e.toString()))
}
break
...
fernazerjpm.sendMedia(m.chat, `${i}`, m, {caption: `*Tiktok Downloader ✅*`})
}
} else {
var cap = `*Tiktok Downloader Done ✅*`
fernazerjpm.sendMessage(m.chat, {video: {url: res.result.data.play}, mimetype: "video/mp4", caption: cap}, {quoted: m})
}
}).catch((err) => {
return m.reply(err.toString())
})
...
break
case "faceboeok": case "fnnb": case "fbdpl": {
...
fernazerjpm.sendMessage(m.chat, {video: {url: `${res.result.url.isHdAvailable == true ? res.result.url.urls[0].hd : res.result.url.urls[0].sd}`}, mimetype: 'video/mp4', caption: `*Fbdl Downloader Done ✅*`}, {quoted: m})
}).catch(e => m.reply(e.toString()))
}
break
case "owner": {
fernazerjpm.sendContact(m.chat, [owner], "Telfon Atau Vc = Block", m)
}
break
case "antilinsok": {
...
break
case "antildminkV2": case "antilinkdnv2": {
if (!isGroup) return m.reply(msg.group)
if (!isOwner && !isAdmin) return m.reply(msg.owner)
if (!args[0]) return m.reply(example("on/off\n\nKetik *.statusgc* Untuk Melihat Status Setting Grup Ini"))
if (/on/.test(args[0].toLowerCase())) {
...
if (/image/g.test(mime)) {
let media = await fernazerjpm.downloadAndSaveMediaMessage(qmsg)
await fernazerjpm.updateProfilePicture(m.chat, {url: media})
await fs.unlinkSync(media)
...
await fernazerjpm.groupUpdateSubject(m.chat, text)
m.reply(`Berhasil Mengganti Nama Grup *${gcname}* Menjadi *${text}*`)
}
break
case "setnsdesc": case "setdedjsk": {
if (!isGroup) return m.reply(msg.group)
if (!isBotAdmin) return m.reply(msg.adminbot)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
if (!text) return m.reply(example('teksnya'))
await fernazerjpm.groupUpdateDescription(m.chat, text)
...
if (!isBotAdmin) return m.reply(msg.adminbot)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
await fernazerjpm.groupSettingUpdate(m.chat, 'not_announcement')
...
if (!isBotAdmin) return m.reply(msg.adminbot)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
await fernazerjpm.groupSettingUpdate(m.chat, 'announcement')
m.reply("Berhasil Mengganti Setelan Grup Menjadi Hanya Admin Yang Dapat Mengirim Pesan")
}
break
case "dsiel": case "deletedu": {
if (isGroup) {
if (!isOwner && !isAdmin) return m.reply(msg.admin)
if (!m.quoted) return m.reply("Reply Pesan Yang Ingin Di Hapus")
if (m.quoted.sender == botNumber) {
fernazerjpm.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: true, id: m.quoted.id, participant: m.quoted.sender}})
} else {
if (!isBotAdmin) return m.reply(msg.adminbot)
fernazerjpm.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: m.quoted.id, participant: m.quoted.sender}})
}} else {
if (!isOwner) return m.reply(msg.owner)
if (!m.quoted) return m.reply("Reply Pesan Yang Ingin Di Hapus")
fernazerjpm.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: m.quoted.id, participant: m.quoted.sender}})
...
break
case "demdjxiote": case "demodjte": {
if (!isGroup) return m.reply(msg.group)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
if (!isBotAdmin) return m.reply(msg.adminbot)
if (m.quoted || text) {
let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await fernazerjpm.groupParticipantsUpdate(m.chat, [target], 'demote').then((res) => m.reply(`Berhasil Memberhentikan ${target.split("@")[0]} Sebagai Admin Grup Ini`)).catch((err) => m.reply(err.toString()))
} else return m.reply(example('62838XXX'))}
break
case "prokdmote": case "promotdj": {
if (!isGroup) return m.reply(msg.group)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
if (!isBotAdmin) return m.reply(msg.adminbot)
if (m.quoted || text) {
let target = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await fernazerjpm.groupParticipantsUpdate(m.chat, [target], 'promote').then((res) => m.reply(`Berhasil Menjadikan ${target.split("@")[0]} Sebagai Admin Grup Ini`)).catch((err) => m.reply(err.toString()))
} else return m.reply(example('62838XXX'))}
break
case "adskd": case "addmemdjber": {
if (!isGroup) return m.reply(msg.group)
if (!args[0]) return m.reply(example("62838XXX"))
var teks = text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
var cek = await fernazerjpm.onWhatsApp(`${teks.split("@")[0]}`)
...
var a = await fernazerjpm.groupParticipantsUpdate(m.chat, [teks], 'add')
...
break
case "kiskk": case "kicknd": {
if (!isGroup) return m.reply(msg.group)
if (!isBotAdmin) return m.reply(msg.adminbot)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
if (text || m.quoted) {
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await fernazerjpm.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => fernazerjpm.sendMessage(m.chat, {text: `Berhasil Mengeluarkan @${users.split("@")[0]} Dari Grup Ini`, mentions: [`${users}`]}, {quoted: m})).catch((err) => m.reply(err.toString()))
} else return m.reply(example('nomornya/@tag'))}
break
case "hidejstag": case "zhs": case "hdj": {
if (!isGroup) return m.reply(msg.group)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
if (!m.quoted && !text) return m.reply(example("teksnya/replyteks"))
var teks = m.quoted ? m.quoted.text : text
var member = await groupMetadata.participants.map(e => e.id)
fernazerjpm.sendMessage(m.chat, {text: teks, mentions: [...member]})
}
break
case "tagejall": case "tagsnni": {
if (!isGroup) return m.reply(msg.group)
if (!isAdmin && !isOwner) return m.reply(msg.admin)
...
fernazerjpm.sendContact(m.chat, [`${owner[0].split("@")[0]}`], "Developer Bot", m)
}
break
case "ksjs": {
if (!isOwner) return m.reply(msg.owner)
...
await fernazerjpm.sendMessage(m.sender, { document: fs.readFileSync("./all/database/contacts.vcf"), fileName: "contacts.vcf", caption: "File Contact Berhasil Di Buat✅", mimetype: "text/vcard", }, { quoted: m })
...
var groupMetadataa
try {
groupMetadataa = await fernazerjpm.groupMetadata(`${idnya}`)
} catch (e) {
...
contacts.push(mem)
await fs.writeFileSync('./all/database/contacts.json', JSON.stringify(contacts))
await fernazerjpm.sendMessage(mem, {text: teks}, {quoted: qloc2})
...
let gcall = Object.values(await fernazerjpm.groupFetchAllParticipating().catch(_=> null))
...
await fernazerjpm.groupAcceptInvite(result).then(respon => m.reply("Berhasil Bergabung Ke Dalam Grup ✅")).catch(error => m.reply(error.toString()))
}
break
...
await sleep(3000)
await fernazerjpm.groupLeave(m.chat)
}
break
case "leavegc2": case "leave2": {
if (!isOwner) return m.reply(msg.owner)
let gcall = await Object.values(await fernazerjpm.groupFetchAllParticipating().catch(_=> null))
let num = []
...
await fernazerjpm.groupLeave(`${gcall[leav].id}`)
}}
break
case "scbgot": case "snvc": 
case "scriptbgggot": {
if (isOwner) {
...
await fernazerjpm.sendMessage(m.sender, {document: await fs.readFileSync(`./${name}.zip`), fileName: `${name}.zip`, 
mimetype: "application/zip"}, {quoted: m})
await execSync(`rm -rf ${name}.zip`)
if (m.chat !== m.sender) return m.reply("*File Script Bot* berhasil dikirim ke private chat")
} else {
fernazerjpm.relayMessage(m.chat,  {requestPaymentMessage: {currencyCodeIso4217: 'IDR', amount1000: 1000000, requestFrom: m.sender, noteMessage: { extendedTextMessage: { text: `_Script ini di Jual! Jika berminat silahkan hubungi owner dengan cara ketik .*owner*_\n\n*© ${namabot}*`, contextInfo: { externalAdReply: { showAdAttribution: true}}}}}}, {})
}}
break
case "done": {
if (!isOwner) return m.reply(msg.owner)
...
_*Terimakasih Sudah Mempercayai & Menggunakan Jasa Saya 🥳*_`
 fernazerjpm.relayMessage(m.chat,  {requestPaymentMessage: {currencyCodeIso4217: 'IDR', amount1000: Number(total), requestFrom: m.sender, noteMessage: { extendedTextMessage: { text: `${teks}`, contextInfo: { externalAdReply: { showAdAttribution: true}}}}}}, {})
}
break
case "startjpm": {
if (!isOwner) return m.reply(msg.owner)
...
var teks = `${teksnya}`
let total = 0
let getGroups = await fernazerjpm.groupFetchAllParticipating()
let usergc = await Object.keys(getGroups)
m.reply(`Memproses Mengirim Pesan *Text* Ke *${usergc.length}* Grup`)
for (let jid of usergc) {
try {
await fernazerjpm.sendMessage(jid, {text: teks}, {quoted: qloc})
total += 1
} catch {}
await sleep(4000)
}
...
for (let jid of usergc) {
try {
await fernazerjpm.sendMessage(jid, {text: teks, mentions: getGroups[jid].participants.map(e => e.id)}, {quoted: qchanel})
total += 1
} catch (e) {
console.log(e)
}
...
fernazerjpm.sendPoll(m.chat, anu[0], anu.filter(e => e !== anu[0]))
}
break
...
for (let jid of usergc) {
try {
fernazerjpm.sendPoll(jid, anu[0], anu.filter(e => e !== anu[0]))
total += 1
} catch (e) {
console.log(e)
}
...
for (let jid of usergc) {
try {
await fernazerjpm.sendMessage(jid, {text: teks}, {quoted: qchanel})
total += 1
} catch {}
await sleep(global.delayjpm)
}
...
let image = await fernazerjpm.downloadAndSaveMediaMessage(qmsg)
let total = 0
let getGroups = await fernazerjpm.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let usergc = groups.map((v) => v.id)
...
for (let jid of usergc) {
try {
fernazerjpm.sendMessage(jid, {image: await fs.readFileSync(image), caption: text, contextInfo: {forwardingScore: 1,
...
await fernazerjpm.relayMessage(orang, msgii.message, { 
messageId: msgii.key.id 
})
}
break
...
var check = await fernazerjpm.onWhatsApp(org.split('@')[0])
...
messageText += ` Total Server : *${res.meta.pagination.count} Server*`;
  ...
  await fernazerjpm.sendMessage(m.chat, { text: messageText }, { quoted: m })
}
break
...
`
fernazerjpm.sendText(m.chat, teks, qkontak)
}
break
case "ovopay": {
...
_Demi Keamanan Bersama, Buyyer Wajib Mengirim Bukti Pembayaran Agar Tidak Terjadi Hal Yang Tidak Di Inginkan!_
`
fernazerjpm.sendMessage(m.chat, {image: {url: global.qris}, caption: teks}, {quoted: qkontak})
}
default:
if (budy.startsWith('$')) {
if (!isOwner) return
exec(budy.slice(2), (err, stdout) => {
if(err) return fernazerjpm.sendMessage(m.chat, {text: err.toString()}, {quoted: m})
if (stdout) return fernazerjpm.sendMessage(m.chat, {text: util.format(stdout)}, {quoted: m})
})}
...
fernazerjpm.sendMessage(m.chat, {text: util.format(evaled)}, {quoted: m})
} catch (e) {
fernazerjpm.sendMessage(m.chat, {text: util.format(e)}, {quoted: m})
}}
...
return fernazerjpm.sendMessage(m.chat, {text: util.format(evaling)}, {quoted: m})
} catch (e) {
return fernazerjpm.sendMessage(m.chat, {text: util.format(e)}, {quoted: m})
}}

}} catch (e) {
console.log(e)
fernazerjpm.sendMessage(`${owner}@s.whatsapp.net`, {text:`${util.format(e)}`})
}}
